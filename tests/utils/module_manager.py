...
class ModuleManager:
    ...
    def update_module_test(self, module_name):
        ...
        module_path = self.apps_dir / module_name
        if not module_path.exists():
            print(f"⚠️  Module {module_name} no longer exists, marking test for cleanup...")
    ...
    def remove_module_test(self, module_name):
        ...
        if test_file.exists():
            ...
            backup_file = test_file.with_suffix(f'.py.backup.{datetime.now().strftime("%Y%m%d_%H%M%S")}')
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == 'sync':
            ...
            print("Usage: python module_manager.py [sync|create|update|remove|generate|cleanup] [module_name]")
    else:
        manager.sync_tests()
