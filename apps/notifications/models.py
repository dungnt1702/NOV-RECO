...
class Notification(models.Model):
    ...
    type = models.CharField(
        max_length=50, choices=NOTIFICATION_TYPES, help_text="Loại thông báo"
    )
    ...
    related_object_id = models.PositiveIntegerField(
        null=True, blank=True, help_text="ID đối tượng liên quan"
    )
    related_object_type = models.CharField(
        max_length=100, blank=True, help_text="Loại đối tượng liên quan"
    )
    ...
    expires_at = models.DateTimeField(
        null=True, blank=True, help_text="Thời gian hết hạn"
    )
    ...
    def get_unread_notifications(cls, user, limit=10):
        ...
        return cls.objects.filter(user=user, is_read=False).order_by("-created_at")[
            :limit
        ]
