...
def location_detail_view(request, location_id):
    ...
    checkins = Checkin.objects.filter(location=location).select_related('user').order_by('-created_at')[:10]
...
def location_checkin_history_view(request, location_id):
    ...
    from apps.checkin.models import Checkin
    checkins = Checkin.objects.filter(location=location).select_related('user').order_by('-created_at')
