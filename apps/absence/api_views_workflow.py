    ...
    if request.method == "GET":
        ...
        try:
            ...
            workflow = ApprovalWorkflow.objects.create(
                ...
                requires_department_manager=data.get(
                    "requires_department_manager", False
                ),
                requires_department_deputy=data.get(
                ...
                department_manager_timeout_hours=data.get(
                    "department_manager_timeout_hours", 24
                ),
                department_deputy_timeout_hours=data.get(
                    "department_deputy_timeout_hours", 24
                ),
                office_director_timeout_hours=data.get(
            ...
            return Response(
                {"success": False, "message": f"Lỗi khi tạo workflow: {str(e)}"},
...
def workflow_detail_api(request, workflow_id):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            workflow.requires_department_manager = data.get(
                "requires_department_manager", False
            )
            workflow.requires_department_deputy = data.get(
                "requires_department_deputy", False
            )
            workflow.requires_office_director = data.get(
            ...
            workflow.department_manager_timeout_hours = data.get(
                "department_manager_timeout_hours", 24
            )
            workflow.department_deputy_timeout_hours = data.get(
                "department_deputy_timeout_hours", 24
            )
            workflow.office_director_timeout_hours = data.get(
                "office_director_timeout_hours", 48
            )
            workflow.office_deputy_timeout_hours = data.get(
                "office_deputy_timeout_hours", 48
            )
            workflow.hr_timeout_hours = data.get("hr_timeout_hours", 48)
            workflow.send_reminder_before_hours = data.get(
            ...
            return Response(
                {"success": True, "message": "Workflow đã được cập nhật thành công"}
            )

        except Exception as e:
            return Response(
                {"success": False, "message": f"Lỗi khi cập nhật workflow: {str(e)}"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            workflow.delete()
            return Response(
                {"success": True, "message": "Workflow đã được xóa thành công"}
            )

        except Exception as e:
            return Response(
                {"success": False, "message": f"Lỗi khi xóa workflow: {str(e)}"},
                status=status.HTTP_400_BAD_REQUEST,
            )
