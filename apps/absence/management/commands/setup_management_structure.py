...
class Command(BaseCommand):
    ...
    def setup_department_management(self, admin_users, dry_run):
        ...
        for department in departments:
            ...
            if not department.deputy_manager and admin_index < len(admin_list):
                ...
                self.stdout.write(f'  → Gán Phó phòng: {deputy_manager.get_full_name()}')
                if not dry_run:
    ...
    def get_management_summary(self):
        ...
        offices_with_deputy = Office.objects.filter(deputy_director__isnull=False).count()
        departments_with_manager = Department.objects.filter(manager__isnull=False).count()
        departments_with_deputy = Department.objects.filter(deputy_manager__isnull=False).count()
