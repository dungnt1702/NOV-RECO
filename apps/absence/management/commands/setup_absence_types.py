...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not dry_run:
            ...
            self.stdout.write(
                self.style.WARNING('Dry run hoàn thành. Chạy lại không có --dry-run để thực hiện.')
            )
    ...
    def create_absence_types(self, dry_run):
        ...
        for absence_data in absence_types:
            absence_type, created = AbsenceType.objects.get_or_create(
                code=absence_data['code'],
                defaults=absence_data
            )
    ...
    def update_special_workflows(self, dry_run):
        ...
        if maternity_type:
            ...
            for workflow in workflows:
                ...
                if not dry_run:
                    workflow.save()
                self.stdout.write(f'  ✓ Cập nhật workflow nghỉ thai sản: {workflow.department.full_name}')
        ...
        if half_day_type:
            ...
            for workflow in workflows:
                ...
                if not dry_run:
                    workflow.save()
                self.stdout.write(f'  ✓ Cập nhật workflow nghỉ nửa ngày: {workflow.department.full_name}')
        ...
        if business_trip_type:
            ...
            for workflow in workflows:
                ...
                if not dry_run:
                    workflow.save()
                self.stdout.write(f'  ✓ Cập nhật workflow nghỉ công tác: {workflow.department.full_name}')
