...
from .models import AbsenceRequest, AbsenceType, ApprovalWorkflow
...
def absence_requests_api(request):
    ...
    if request.user.is_superuser or request.user.is_admin_user:
        absence_requests = AbsenceRequest.objects.select_related(
            "user", "absence_type", "workflow"
        ).all()
    else:
        absence_requests = AbsenceRequest.objects.select_related(
            "user", "absence_type", "workflow"
        ).filter(Q(user=request.user) | Q(current_approver=request.user))
    ...
    data = [
        {
            ...
            "current_approver": (
                ar.current_approver.get_full_name() if ar.current_approver else None
            ),
...
def approve_request_api(request, absence_id):
    ...
    try:
        ...
        return Response({"success": True, "message": f"Đơn vắng mặt đã được {action}"})
        ...
        return Response(
            {
                ...
                "current_approver": (
                    absence_request.current_approver.get_full_name()
                    if absence_request.current_approver
                    else None
                ),
