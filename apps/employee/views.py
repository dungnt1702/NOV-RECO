...
def employee_create_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            user.save()
            messages.success(
                request, f"Tạo nhân viên {user.get_full_name()} thành công!"
            )
...
def employee_update_view(request, employee_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(
                request, f"Cập nhật thông tin {employee.get_full_name()} thành công!"
            )
            return redirect("employee:list")
    else:
...
def employee_detail_view(request, employee_id):
    ...
    checkins = (
        Checkin.objects.filter(user=employee)
...
def employee_toggle_active_view(request, employee_id):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request, f"Đã {status_text} nhân viên {employee.get_full_name()}"
        )
...
def employee_statistics_view(request, employee_id):
    ...
    today_checkins = Checkin.objects.filter(
        user=employee, created_at__date=today
    ).count()

    week_checkins = Checkin.objects.filter(
        user=employee, created_at__date__gte=week_start
    ).count()

    month_checkins = Checkin.objects.filter(
        user=employee, created_at__date__gte=month_start
    ).count()
