...
from .serializers import CheckinListSerializer, CheckinCreateSerializer
...
def checkin_submit_view(request):
    ...
    try:
        ...
        if not photo:
            return JsonResponse({'success': False, 'error': 'Vui lòng chụp ảnh'}, status=400)
        ...
        if not area:
            ...
            for a in areas:
                ...
                if distance < min_distance and distance <= a.radius_m:
...
def checkin_history_view(request):
    """Lịch sử check-in"""
    checkins = Checkin.objects.filter(user=request.user).select_related('area').order_by('-created_at')
...
def checkin_list_view(request):
    ...
    checkins = Checkin.objects.select_related('user', 'area').order_by('-created_at')
