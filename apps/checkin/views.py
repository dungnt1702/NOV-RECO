...
from .utils import find_best_location_for_checkin, haversine_distance
...
def checkin_submit_view(request):
    ...
    try:
        ...
        checkin_type = request.POST.get(
            "checkin_type", "1"
        )  # Default to '1' (Chấm công)
        ...
        # Tính khoảng cách
        if location:
            checkin.distance_m = haversine_distance(
                location.lat, location.lng, lat, lng
            )
            checkin.save()

        # New pretty URL: /checkin/success/checkin_id/<id>/
        redirect_url = reverse("checkin:success", kwargs={"checkin_id": checkin.id})

        return JsonResponse(
            {
                "success": True,
                "checkin_id": checkin.id,
                "location_name": (location.name if location else "Không xác định"),
...
def checkin_success_view(request, checkin_id):
    ...
    try:
        ...
        success_data = {
            ...
            "user_department": (
                checkin.user.department.name if checkin.user.department else "N/A"
            ),
            ...
            "checkin_time": timezone.localtime(checkin.created_at).strftime(
                "%d/%m/%Y %H:%M:%S"
            ),
        ...
        return render(
            request, "checkin/success.html", {"error": f"Có lỗi xảy ra: {str(e)}"}
        )
...
def checkin_list_view(request):
    ...
    checkins = Checkin.objects.select_related("user").order_by("-created_at")
...
def checkin_history_api(request):
    ...
    if search:
        checkins = checkins.filter(
            Q(note__icontains=search) | Q(location__name__icontains=search)
        )
