...
def checkout_action_view(request):
    ...
    if checkin_id:
        ...
        latest_checkin = Checkin.objects.filter(
            ...
            checkouts__isnull=False  # Loại bỏ checkin đã có checkout
...
def checkout_submit_view(request):
    ...
    try:
        ...
        if not checkin_id:
            return JsonResponse(
                {'success': False, 'error': 'Không tìm thấy thông tin checkin'}, status=400
            )
        ...
        if Checkout.objects.filter(checkin=checkin).exists():
            return JsonResponse(
                {'success': False, 'error': 'Checkin này đã có checkout rồi'}, status=400
            )
        ...
        return JsonResponse(
            {'success': False, 'error': f'Lỗi: {str(e)}'}, status=500
        )
...
def checkout_success_view(request, checkout_id):
    ...
    try:
        ...
        success_data = {
            ...
            'user_department': (
                checkout.user.department.name
                if checkout.user.department else 'N/A'
            ),
            'user_employee_id': checkout.user.username,
            'location_name': checkout.get_location_name(),
            'coordinates': (
                f"{checkout.lat:.6f}, {checkout.lng:.6f}"
            ),
            'address': checkout.address or 'Không xác định',
            'created_at': checkout.created_at.strftime('%d/%m/%Y %H:%M:%S'),
            'note': checkout.note or '',
            'photo_url': (
                checkout.photo.url if checkout.photo else None
            ),
        ...
        return render(request, 'checkin/checkout_success.html', {
            'error': f'Lỗi: {str(e)}'
        })
...
def checkout_detail_view(request, checkin_id):
    ...
    try:
        ...
        return render(request, 'checkin/checkout_detail.html', {
            'error': f'Lỗi: {str(e)}'
        })
...
def checkout_list_view(request):
    ...
    checkouts = (Checkout.objects.select_related('user', 'checkin')
                .order_by('-created_at'))
...
def checkout_history_api(request):
    ...
    if search:
        checkouts = checkouts.filter(
            Q(note__icontains=search) |
            Q(checkin__note__icontains=search)
        )
