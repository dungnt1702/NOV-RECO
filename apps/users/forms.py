...
class UserUpdateForm(forms.ModelForm):
    ...
    emergency_contact = forms.CharField(
        max_length=100, required=False, label="Người liên hệ khẩn cấp"
    )
    emergency_phone = forms.CharField(
        max_length=20, required=False, label="Số điện thoại liên hệ khẩn cấp"
    )
    position = forms.CharField(max_length=100, required=False, label="Chức vụ")
    hire_date = forms.DateField(required=False, label="Ngày vào làm")
    salary = forms.DecimalField(
        max_digits=12, decimal_places=2, required=False, label="Lương"
    )
    work_schedule = forms.CharField(
        max_length=100, required=False, label="Lịch làm việc"
    )
    ...
    is_active_employee = forms.BooleanField(
        required=False, label="Nhân viên đang hoạt động"
    )
...
class DepartmentForm(forms.ModelForm):
    ...
    def clean_name(self):
        ...
        if name and office:
            ...
            if self.instance.pk:
                if (
                    Department.objects.filter(name=name, office=office)
                    .exclude(pk=self.instance.pk)
                    .exists()
                ):
                    raise forms.ValidationError(
                        f'Tên phòng ban "{name}" đã tồn tại trong {office.name}'
                    )
            else:
...
class OfficeForm(forms.ModelForm):
    ...
    def clean_name(self):
        ...
        if name:
            ...
            if self.instance.pk:
                if (
                    Office.objects.filter(name=name)
                    .exclude(pk=self.instance.pk)
                    .exists()
                ):
