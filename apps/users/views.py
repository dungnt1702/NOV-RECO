...
from apps.users.forms import DepartmentForm, OfficeForm, UserCreateForm, UserUpdateForm
from apps.users.models import Department, Office, User, UserRole
...
def user_list_view(request, office_id=None, department_id=None):
    ...
    if dept_filter and not office_filter:
        dept_obj = (
            Department.objects.select_related("office").filter(id=dept_filter).first()
        )
    ...
    departments = Department.objects.select_related("office").order_by(
        "office__name", "name"
    )
...
def user_create_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            user = form.save()
            messages.success(
                request, f"Tạo người dùng {user.get_full_name()} thành công!"
            )
...
def user_import_view(request):
                ...
                try:
                    ...
                    if department_id not in (None, ""):
                        try:
                            ...
                            errors.append(
                                f"Dòng {idx}: department_id {department_id} không tồn tại"
                            )
                            dept_obj = None
                    if (
                        not dept_obj
                        and assign_default_department
                        and default_department_id
                    ):
                        try:
                            dept_obj = Department.objects.get(
                                id=int(default_department_id)
                            )
                        except Department.DoesNotExist:
                            dept_obj = None
    ...
    try:
        ...
        if [h.lower() for h in headers] != expected_headers:
            messages.error(
                request,
                "Tiêu đề cột CSV không hợp lệ. Yêu cầu: " + ", ".join(expected_headers),
            )
            return redirect("users:import")
        process_rows(reader)
    except Exception as e:
    ...
    if created_count or updated_count:
        messages.success(
            request,
            f"Import thành công: tạo {created_count}, cập nhật {updated_count}.",
        )
    if errors:
        messages.warning(
            request,
...
def user_update_view(request, id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(
                request, f"Cập nhật thông tin {target_user.get_full_name()} thành công!"
            )
            return redirect("users:list")
    else:
...
def department_update_view(request, dept_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            department = form.save()
            messages.success(
                request, f"Cập nhật phòng ban {department.name} thành công!"
            )
            return redirect("users:department_list")
    else:
...
def office_create_view(request):
    ...
    users = User.objects.filter(groups__name__in=["Admin", "Manager"]).order_by(
        "first_name", "last_name"
    )
    return render(request, "users/office_create.html", {"form": form, "users": users})
