...
from .models import User, Department, Office
        ...
        if office_id:
            try:
                ...
                if user.can_view_office_data(office):
                    ...
                    messages.error(request, 'Bạn không có quyền truy cập dữ liệu của văn phòng này.')
        ...
        if department_id:
            try:
                ...
                if user.can_view_department_data(department):
                    ...
                    messages.error(request, 'Bạn không có quyền truy cập dữ liệu của phòng ban này.')
...
def hr_or_manager_required(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        ...
        if (user.is_office_director() or user.is_office_deputy_director() or 
            user.is_department_manager() or user.is_department_deputy_manager()):
            return view_func(request, *args, **kwargs)
...
def data_isolation_mixin(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        ...
        request.user_permissions = {
            ...
            'managed_offices': list(user.get_managed_offices().values_list('id', flat=True)),
            'managed_departments': list(user.get_managed_departments().values_list('id', flat=True)),
        }
