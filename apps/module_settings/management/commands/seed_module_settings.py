...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if options["reset"]:
            ...
            self.stdout.write(self.style.WARNING("Đã xóa tất cả module settings"))

        # Lấy superuser đầu tiên làm created_by
        superuser = User.objects.filter(is_superuser=True).first()
        if not superuser:
            self.stdout.write(self.style.ERROR("Không tìm thấy Super admin nào!"))
            return
            ...
            try:
                ...
                if created:
                    ...
                    if updated:
                        ...
                        self.stdout.write(
                            self.style.WARNING(
                                f"↻ Cập nhật module: {module.display_name}"
                            )
                        )
                    else:
                        self.stdout.write(f"  Module đã tồn tại: {module.display_name}")
        ...
        # Kết quả
        self.stdout.write("\n" + "=" * 50)
        self.stdout.write(self.style.SUCCESS(f"Tổng kết:"))
