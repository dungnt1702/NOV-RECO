        ...
        def wrapper(request, *args, **kwargs):
            if not ModuleSettings.is_module_enabled(module_name):
                if request.headers.get("Accept") == "application/json":
                    return JsonResponse(
                        {"error": "Module này đã bị tắt", "module": module_name},
                        status=403,
                    )

                # Render trang thông báo module bị tắt
                context = {"module_name": module_name, "title": "Module không khả dụng"}
                return render(
                    request, "module_settings/disabled.html", context, status=403
                )
