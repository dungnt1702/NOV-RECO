...
def get_sales_data(user):
    ...
    data = {
        ...
        "table": [
            ...
            {
                "product": "Sản phẩm B",
                "sales": daily_sales * 0.8,
                "units": int(daily_checkins * 0.8),
            },
            {
                "product": "Sản phẩm C",
                "sales": daily_sales * 0.6,
                "units": int(daily_checkins * 0.6),
            },
        ],
    }
...
def get_hr_data(user):
        ...
        dept_data.append(
            {
                ...
                "percentage": (
                    round((dept.employee_count / total_employees * 100), 1)
    ...
    for office in offices:
        office_employees = User.objects.filter(
            department__office=office, is_active=True
        ).count()
        office_data.append({"name": office.name, "count": office_employees})
    ...
    data = {
        ...
        "metrics": {
            ...
            "avg_employees_per_dept": (
                round(total_employees / departments.count(), 1)
...
def get_marketing_data(user):
    ...
    campaigns = [
        {
            "name": "Campaign A",
            "reach": daily_checkins * 10,
            "engagement": daily_checkins * 2,
            "conversion": daily_checkins * 0.1,
        },
        {
            "name": "Campaign B",
            "reach": weekly_checkins * 5,
            "engagement": weekly_checkins * 1.5,
            "conversion": weekly_checkins * 0.08,
        },
        {
            "name": "Campaign C",
            "reach": daily_checkins * 8,
            "engagement": daily_checkins * 1.8,
            "conversion": daily_checkins * 0.12,
        },
    ]
...
def get_finance_data(user):
    ...
    data = {
        ...
        "metrics": {
            ...
            "profit": profit,
            "profit_margin": (
                f"{(profit/monthly_revenue*100):.1f}%" if monthly_revenue > 0 else "0%"
            ),
        },
        "table": [
            {"category": "Revenue", "amount": monthly_revenue, "percentage": 100},
            {
                ...
                "percentage": (
                    round(monthly_expenses / monthly_revenue * 100, 1)
            ...
            },
            {
                ...
                "amount": profit,
                "percentage": (
                    round(profit / monthly_revenue * 100, 1)
                    if monthly_revenue > 0
                    else 0
                ),
            },
...
def get_operations_data(user):
    ...
    area_data = []
    for area in areas:
        area_checkins = Checkin.objects.filter(
            area=area, created_at__date=today
        ).count()
        area_data.append(
            {
                "name": area.name,
    ...
    data = {
        ...
        "metrics": {
            ...
            "avg_efficiency": (
                round(sum(area["efficiency"] for area in area_data) / len(area_data), 1)
                if area_data
                else 0
            ),
            "utilization_rate": "85%",
